...
internal fun File.zipTo(zipFile: File = File(parentFile, "$name.jar")) {
    zipTo(zipFile, this, walkReproducibly())
}

internal fun File.unzipTo(outputDirectory: File = File(parent, nameWithoutExtension)): File {
...
internal fun File.walkReproducibly(): Sequence<File> = sequence {
...
internal fun ZipFile.hasEntry(className: String): Boolean {
